...
.service('Funds', function($q, Fund, Loading) {
	...
	var fundsRef = firebase.database().ref('/funds');
  ...
  this.getAll = function(uid) {
    ...
    var promise = fundsRef.orderByValue().ref
      .orderByChild('uid').equalTo(uid)
      .once('value').then(function(snapshot) {
        var funds = [];
        snapshot.forEach(function(data) {
          funds.push(_getFund(data));
        });
  ...
  this.setFund = function(fund) {
    var fundRef;
    if (fund.id) {
      fundRef = fundsRef.child(fund.id);
    } else {
      fundRef = fundsRef.push();
    }

    fund.id = fundRef.key;
    var info = fund.getInfo();
    var promise = fundRef.set(info);

    return Loading.progress(promise);
  }

  /**
   * [_getFund description]
   * @param  {[type]} data [description]
   * @return {[type]}      [description]
   */
  function _getFund(data) {
    var info = data.val();
    info.id = data.key;
    return new Fund(info);
  }

})

.factory('Fund', function(Auth) {
  ...
  Fund.prototype.getInfo = function() {
    return {
      uid: this.uid,
